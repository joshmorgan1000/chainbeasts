...
inline bool http_get(const std::string& host, unsigned short port, const std::string& path,
                     std::string* response = nullptr, int attempts = 3, int backoff_ms = 100) {
#if defined(_WIN32) || defined(__unix__)
    harmonics::net_init();
    int delay = backoff_ms;
    for (int attempt = 0; attempt < attempts; ++attempt) {
        ...
        if (pos != std::string::npos && chk.size() >= pos + 12) {
            ...
            auto header_end = chk.find("\r\n\r\n", pos);
            if (header_end != std::string::npos)
                chk.erase(0, header_end + 4);
        }
        return true;
    }
    std::cerr << "http_get failed after " << attempts << " attempts" << std::endl;
    return false;
#else
    (void)host;
    (void)port;
